class Game extends Phaser.Scene {
  ...
  preload() {
    ...
    for (let i = 1; i <= 152; ++i) {
      this.load.image(`devil${i}`, `Devil Emoji/fd1abf7e-069b-42d6-8022-72baf150dc66-${i - 1}.png`);
    }

    for (let i = 1; i <= 81; ++i) {
      this.load.image(`bird${i}`, `Cute Bird/24bd7ff8-213a-41ca-959e-b9d509cf1132-${i - 1}.png`);
    }
  ...
  addHomeUI() {
    ...
    this.optionsContainer = this.add.rexRoundRectangle(400, 900, 620, 480, 50, 0xffffff).setDepth(5).setScrollFactor(0);

    this.birdImage = this.add.image(670, 690, "bird18").setScale(0.5).setDepth(Infinity);
    ...
    this.option1 = this.add.rexRoundRectangle(400, 830, 520, 100, 50, 0xf3e3a3).setDepth(5).setScrollFactor(0).setInteractive();
    ...
    this.option2 = this.add.rexRoundRectangle(400, 945, 520, 100, 50, 0xfaa7ab).setDepth(5).setScrollFactor(0).setInteractive();
    ...
    this.option3 = this.add.rexRoundRectangle(400, 1060, 520, 100, 50, 0x4e316e).setDepth(5).setScrollFactor(0).setInteractive();
  ...
  addRestartUI() {
    ...
    this.termsText = this.add
      .text(400, 1170, "Powered by Rabble. By playing this game you accept these Terms & policies.", {
        fontFamily: "RakeslyRG",
        fontSize: "20px",
        color: "#000",
        align: "center",
  ...
  addCodesUI() {
    ...
    this.rabbleButton = this.add.rexRoundRectangle(400, 1060, 420, 100, 50, 0x4e316e).setDepth(5).setScrollFactor(0).setInteractive();
  ...
  addBackground() {
    this.background = this.add.image(400, 600, "background").setScale(1.2).setScrollFactor(0).setDepth(0);
  }
  addGameUI() {
  ...
  addScores() {
    ...
    this.scoreBox = this.add.rexRoundRectangle(60, 32, 140, 45, 15, 0x4e316e).setDepth(10).setScrollFactor(0).setOrigin(0);

    this.scoreImage = this.add.image(65, 55, "star").setDepth(Infinity).setScrollFactor(0).setScale(0.6);
  ...
  addLife() {
    this.life = 3;
    this.lifes = [];
    this.lifeBox = this.add.rexRoundRectangle(340, 33, 140, 45, 15, 0x4e316e).setDepth(10).setScrollFactor(0).setOrigin(0);
  ...
  addProgress() {
    this.progressBox = this.add.rectangle(340, 33, 140, 45, 0x66ee99).setDepth(11).setScrollFactor(0).setOrigin(0);
    this.progressMask = this.add.rexRoundRectangle(340, 33, 140, 45, { tl: 15, tr: 15, bl: 15, br: 15 }, 0xffffff).setDepth(9).setScrollFactor(0).setOrigin(0);
    this.progressBox.setMask(this.progressMask.createGeometryMask());
  }
  start() {
    this.playing = true;
    this.devilGroup = this.physics.add.group();
    ...
    for (let i = 0; i < this.randomBetween(1, 4); ++i) {
      const cloud = this.physics.add
        .image(this.randomBetween(-200, 1000), this.randomBetween(50, 600), `cloud${this.randomBetween(1, 3)}`)
        .setScrollFactor(1, 0)
        .setDepth(1);
  ...
  setup() {
    this.ground = this.physics.add.image(1200, 1020, "ground").setScale(1).setVelocityX(20);
  }
  createPlayer() {
    this.player = this.physics.add.sprite(400, 300, "bird1").setScale(0.8).setDepth(4).setCircle(30, 10, 5).setAngle(10);
    ...
    if (!this.player.lost) {
      if (this.progressDecreasing) {
        if (this.progressWidth > 0) {
          ...
          this.progressDecreasing = false; // Switch to refilling
        }
      } else {
  ...
  switchTo() {
    this.progressDevil = !this.progressDevil;
  }
  addClouds() {
    this.clouds1 = [];
  ...
  generateClouds() {
    for (let i = 0; i < this.randomBetween(1, 4); ++i) {
      const cloud = this.physics.add
        .image(this.player.x + 800, this.randomBetween(50, 600), `cloud${this.randomBetween(1, 3)}`)
        .setScrollFactor(1, 0)
        .setDepth(1);
  ...
  addDevilsAndProducts() {
    ...
    this.time.addEvent({
      delay: (this.randomBetween(1000, 2000) * 100) / this.player.speed,
  ...
  generateDevilAndProducts() {
    ...
    let productX = this.player.x + 700 + this.randomBetween(1, 50);
    let productY = this.randomBetween(100, 1000);
    this.generateDevil(productX, productY);
    this.generateProduct(productX, productY);
  }
  generateDevil(x, y) {
    const devil = this.devilGroup.create(x, y, "devil1").setScrollFactor(1, 0).setDepth(2).setScale(0.2).setCircle(160, 90, 100);
    devil.anims.play("devilAnimation");

    this.physics.add.overlap(
      this.player,
      devil,
      (player, devil) => {
        if (!this.player.lost && devil.active) {
          if (devil.visible) {
            devil.setActive(false);
            this.life -= 1;
            this.updateLives();
            this.devilTweenDestroy(devil);
          } else {
            devil.destroy();
          }
        }
    ...
    this.physics.add.overlap(this.leftWall, devil, (player, devil) => {
      devil.destroy();
    });
  }
  devilTweenDestroy(devil) {
    this.tweens.add({
      targets: devil,
  ...
  generateProduct(x, y) {
    ...
    const productImage = `product${rnd}`;
    const product = this.productGroup.create(x, y, productImage).setScrollFactor(1, 0).setDepth(2).setScale(0.2).setCircle(160, 90, 100);

    this.physics.add.overlap(
      this.player,
      product,
      (player, product) => {
        if (!this.player.lost && product.active) {
          if (product.visible) {
            product.setActive(false);
            let pointAddText = this.add
              .text(product.x, product.y, `+${this.pointTimes}`, {
                fontSize: "34px",
                color: "#7b46bb",
                fontStyle: "bold",
              })
            ...
            this.productTweenDestroy(product);
          } else {
            product.destroy();
          }
        }
    ...
    this.physics.add.overlap(this.leftWall, product, (wall, product) => {
      product.destroy();
    });
  }
  productTweenDestroy(product) {
    this.tweens.add({
      targets: product,
  ...
  update() {
    if (this.playing) {
      ...
      if (this.progressDevil) {
        this.productGroup.setVisible(false);
        this.devilGroup.setVisible(true);
      } else {
        this.devilGroup.setVisible(false);
